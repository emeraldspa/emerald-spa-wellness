  ...
  return (
    <div
      className={`relative w-full overflow-hidden ${
        state === 'failed' ? 'min-h-0 bg-ground' : 'min-h-[640px] bg-white'
      }`}
    >
      {state === 'loading' ? (
        <div
      ...
      {/*
        ...
        embedded booking feels broken. The client asked for the frame to be as
        big as possible: it takes every pixel below the heading block.
      */}
      {state === 'failed' ? null : (
        <iframe
          key={attempt}
          ...
          className="block h-[calc(100dvh-11.5rem)] min-h-[640px] w-full border-0"
          loading="lazy"
