 ...
 * wordmark (gem SVG on an ivory tile + Radley text), the section links with
 * visual dropdowns, search, the hamburger, and two booking CTAs on desktop.
 * The hamburger opens a full-screen glass drawer with the complete menu,
 * contact and hours; search opens its own overlay. One header, one system,
 * nothing competing.
 */
    ...
    <>
      <div className="fixed inset-x-3 top-3 z-50 sm:inset-x-5 sm:top-4 lg:inset-x-8">
        ...
        */}
        <div className="relative">
          <div
            aria-hidden="true"
            className="pointer-events-none absolute inset-0 rounded-full border border-white/12 bg-[#07211A]/72 shadow-[0_18px_50px_-16px_rgba(7,33,26,0.65)] backdrop-blur-2xl backdrop-saturate-150"
          >
            <div className="rule-gold absolute inset-x-10 bottom-0 h-px opacity-60" />
          </div>
          <nav
            className="relative flex items-center justify-between gap-3 px-3.5 py-2 sm:px-5 md:gap-5"
            aria-label="Primary"
          >
                ...
                <button
                  ...
                  className={`flex min-h-[44px] items-center gap-1.5 rounded-full px-3.5 text-[11px] font-semibold uppercase tracking-[0.16em] transition-colors ${
                    pathname.startsWith('/services')
                      ? 'text-gold-200'
                      : 'text-ground/85 hover:text-gold-200'
                  }`}
                >
                  Services
                  <ChevronDown
                  ...
                  <li key={item.href}>
                    <Link
                      ...
                      className={`flex min-h-[44px] items-center rounded-full px-3.5 text-[11px] font-semibold uppercase tracking-[0.16em] transition-colors ${
                        active ? 'text-gold-200' : 'text-ground/85 hover:text-gold-200'
                      }`}
                    >
                      {item.label}
                    </Link>
      ...
      <AnimatePresence>
        {drawerOpen ? (
          <motion.div
            id="site-drawer"
            className="fixed inset-0 z-[60] overflow-y-auto bg-[#062018]/93 backdrop-blur-2xl"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
      ...
      <SearchOverlay open={searchOpen} onClose={() => setSearchOpen(false)} journalPosts={journalPosts} />
    </>
  );
}
    ...
    <AnimatePresence>
      {open ? (
        <motion.div
          ...
          className="absolute left-1/2 top-full z-50 mt-3 -translate-x-1/2 rounded-2xl border border-white/12 bg-[#06231B]/95 p-3 shadow-[0_28px_70px_-20px_rgba(0,0,0,0.6)] backdrop-blur-2xl"
        >
          {children}
        </motion.div>
      ) : null}
