 ...
 *
 * This is the site's navigation. The only other nav is the hero's own inline
 * bar on the home page, which scrolls away. This pill appears once the
 * visitor is past the first screen on home; on inner pages it is present
 * from load so there is always a way around without a boxed header.
 *
...
export function StickyNav() {
  ...
  // On home the hero carries the nav, so the pill waits until the hero has
  // scrolled away. Inner pages have no hero nav, so the pill shows at once.
  ...
  const onBooking = pathname === BOOKING_PATH;
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            {!onBooking ? (
              ...
              <a
                ...
                tabIndex={shown ? undefined : -1}
                className="flex min-h-[44px] items-center gap-1.5 rounded-full border border-gold-300/45 px-4 text-[11px] font-semibold uppercase tracking-[0.16em] text-gold-200 transition-colors hover:border-gold-300 hover:bg-gold-300/10 sm:px-5"
              >
                {site.phone}
              </a>
            )}
