...
import {
  ...
  WHATSAPP_PATH,
  getImage,
  site, ogFor } from '@/lib/site';

export const metadata: Metadata = {
  title: 'Services and Prices',
  description: `All ${LISTED_SERVICE_COUNT} treatments at Emerald Spa & Wellness Centre, Windhoek North: massages, facials, hydrotherapy, nails, lashes and hair removal. Prices in Namibian dollars.`,
  alternates: { canonical: '/services' },
  openGraph: ogFor('/services'),
};
  ...
  return (
    <>
      ...
      <main id="main">
                    ...
                    <li key={`${cat.slug}-${s.name}`} className="py-5">
                      ...
                      {s.description ? (
                        <RevealText
                          text={s.description}
                          lines={2}
                          className="mt-2.5 max-w-2xl text-sm leading-relaxed text-ink/65"
                        />
                      ) : null}
        ...
        {/* The emerald, as a closing band: the stone that gave the spa its name. */}
        <section
          className="relative overflow-hidden border-t border-ink/10 bg-emerald-50"
          aria-label="The emerald stone"
        >
          <div
            aria-hidden="true"
            ...
            style={{
              backgroundImage:
                "radial-gradient(ellipse 70% 130% at 88% 15%, rgba(117,224,186,0.35), transparent 55%), url('/media/emerald-stone.webp')",
              backgroundSize: 'auto, 420px',
              backgroundRepeat: 'no-repeat, no-repeat',
              backgroundPosition: 'center, 90% 60%',
            }}
          />
          <div className="shell relative py-16 md:py-20">
            <div className="max-w-xl">
              <p className="eyebrow text-emerald-700">The stone</p>
              <h2 className="display mt-4 text-3xl text-balance sm:text-4xl">
                It all starts with a single emerald.
              </h2>
              <p className="mt-5 leading-relaxed text-ink/75 text-pretty">
                Every treatment on this menu carries the same standard the stone stands for:
                real materials, honest skill, and enough time to actually relax. Pick one, or
                ask the team to build the afternoon around you.
              </p>
            </div>
          </div>
        </section>
