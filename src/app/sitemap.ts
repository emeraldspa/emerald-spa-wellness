...
/**
 * Static route map. Content the editors change (specials) lives on pages
 * that are revalidated from WordPress on a 15-minute window, so the sitemap
 * itself stays a plain static list: one URL per public route, no invented
 * timestamps.
 */
export default function sitemap(): MetadataRoute.Sitemap {
  const routes: { path: string; priority: number; freq: 'weekly' | 'monthly' | 'yearly' }[] = [
    ...
    { path: '/specials', priority: 0.7, freq: 'weekly' },
